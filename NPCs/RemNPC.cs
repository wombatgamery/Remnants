    ...
    public class RemNPC : GlobalNPC
    {
        ...
        public override void EditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo)
        {
            ...
            if (!Main.wallHouse[Main.tile[spawnInfo.SpawnTileX, spawnInfo.SpawnTileY - 1].WallType])
            {
                ...
                else if (spawnInfo.Player.InModBiome(ModContent.GetInstance<Hive>()) || tile.TileType == TileID.Hive || tile.WallType == ModContent.WallType<hive>() || tile.WallType == WallID.HiveUnsafe)
                {
                    pool.Clear();
