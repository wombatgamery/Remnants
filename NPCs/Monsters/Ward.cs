...
using Remnants.Projectiles.Effects;
...
namespace Remnants.NPCs.Monsters
{
    public class Ward : ModNPC
    {
            ...
            if (NPC.collideX && (direction == 2 || direction == 4) || NPC.collideY && (direction == 1 || direction == 3) || direction == -1 || direction == 0)
            {
                ...
                if (collisionFrames == 1)
                {
                    if (direction > 0)
                    {
                        ...
                        {
                            SoundEngine.PlaySound(SoundID.Item62, position: NPC.Center);
        ...
        {
            Projectile.NewProjectileDirect(NPC.GetSource_None(), npc.Center + Vector2.UnitY * yOffset, Vector2.Zero, ModContent.ProjectileType<MazeGuardianTeleport>(), 0, 0);
            SoundEngine.PlaySound(new SoundStyle("Remnants/Sounds/starspawnspellcharge"), npc.Center);
        }
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            {
                int num354 = Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.Stone, hit.HitDirection * 2, -2f, NPC.alpha, default(Color), 1.2f);
                if (Main.rand.NextBool(8))
                {
