    ...
    public class magicallab : ModWall
    {
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (!tile.HasTile || !Main.tileSolid[tile.TileType] || !Main.tileBlockLight[tile.TileType] || tile.Slope != SlopeType.Solid || tile.IsHalfBlock)
            {
                ...
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>(Texture + "glow").Value, position, frame, RemTile.MagicalLabLightColour(j), 0f, Vector2.Zero, 1f, 0, 0f);
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>(Texture + "glow2").Value, position, frame, Color.White, 0f, Vector2.Zero, 1f, 0, 0f);
            }
