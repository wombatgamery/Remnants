...
namespace Remnants.Content.Tiles.SpiderNest
{
            ...
            for (int y = -1; y <= 1; y++)
            {
                for (int x = -1; x <= 1; x++)
                {
                    if (x != 0 || y != 0)
                    {
                        if (CanMerge(i + x, j + y))
                        {
                            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/SpiderNest/SpiderEggMerge").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle((-x + 1) * 16, (-y + 1) * 16, 16, 16), Lighting.GetColor(i, j), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
                        }
                    }
                }
            }
