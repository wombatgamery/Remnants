	...
	public class AlchemyBench : ModTile
	{
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (tile.TileFrameY == 0)
            {
                ...
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Objects/Furniture/AlchemyBenchGlow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16), RemTile.MagicalLabLightColour(i), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Objects/Furniture/AlchemyBenchShine").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
            }
        }
    }
}