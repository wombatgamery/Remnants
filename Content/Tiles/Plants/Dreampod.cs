...
namespace Remnants.Content.Tiles.Plants
{
	...
	public class DreampodVine : ModTile
	{
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Plants/DreampodVineGlow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16), Color.White, 0f, Vector2.Zero, 1f, effect, 0f);
        }
    }
}