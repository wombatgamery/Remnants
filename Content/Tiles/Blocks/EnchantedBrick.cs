	...
	public class EnchantedBrick : ModTile
	{
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			if (!Main.tile[i, j - 1].HasTile || !Main.tileSolid[Main.tile[i, j - 1].TileType] || Main.tile[i, j - 1].TileType == TileID.ClosedDoor)
			{
				...
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Blocks/EnchantedBrickGlow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16), RemTile.MagicalLabLightColour(i), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Blocks/EnchantedBrickShine").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			}
		}
		...
		private void DrawCosmos(int i, int j)
        {
			Tile tile = Main.tile[i, j];
			Vector2 zero = Main.drawToScreen ? Vector2.Zero : new Vector2(Main.offScreenRange, Main.offScreenRange);
			//Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/blocks/labtiles-layer1").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(0, 0, 16, 16), RemTile.MagicalLabLightColour(i, j), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			...
			for (int k = 0; k < 8; k++)
            {
				...
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/blocks/cosmos").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 + k * 2 - (int)Main.screenPosition.Y) + zero, frame, Lighting.GetColor(i, j), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			}
			
			//frame = new((i * 16) % (20 * 16), (int)(j * 16 + Main.GameUpdateCount * 0.25f) % (20 * 16), 16, 16);

			//Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/blocks/labtiles-layer3").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, frame, RemTile.MagicalLabLightColour(i, j), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
		}
