		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			for (int y = -1; y <= 1; y++)
            {
				for (int x = -1; x <= 1; x++)
				{
                    ...
                    {
						Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/Tiles/Blocks/cloudmerge").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle((-x + 1) * 16 + (CanMerge(i + x, j + y) == 2 ? 48 : 0), (-y + 1) * 16, 16, 16), Lighting.GetColor(i, j), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
					}
				}
			}
		}
