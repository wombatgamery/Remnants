	...
	public class SulfuricVent : ModTile
	{
        ...
        public override void EmitParticles(int i, int j, Tile tile, short tileFrameX, short tileFrameY, Color tileLight, bool visible)
        {
			if (!Main.tile[i, j - 1].HasTile || Main.tile[i, j - 1].TileType != Type)
			{
                ...
                if (Main.rand.NextBool(2))
                {
                    Gore gore = Gore.NewGorePerfect(new EntitySource_TileUpdate(i, j), new Vector2(i * 16 - 16 + Main.rand.NextFloat(7f, 9f), j * 16 - 8), new Vector2(0f, Main.rand.NextFloat(0, -4) * strength), Main.rand.Next(220, 222), Main.rand.NextFloat(1, 2));
