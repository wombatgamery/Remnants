    ...
    public class HoneySlime : ModNPC
    {
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life > 0)
            {
                ...
                {
                    int num352 = Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.Honey, hit.HitDirection * 2, -1f, NPC.alpha, default, 1.2f);
                    if (Main.rand.NextBool(8))
                    {
            ...
            {
                int num354 = Dust.NewDust(NPC.position, NPC.width, NPC.height, DustID.Honey, hit.HitDirection * 2, -2f, NPC.alpha, default, 1.2f);
                if (Main.rand.NextBool(8))
                {
