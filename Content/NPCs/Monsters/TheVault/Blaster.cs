...
namespace Remnants.Content.NPCs.Monsters.TheVault
{
    public class Blaster : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            else if (aiState == 1)
            {
                ...
                else
                {
                    ...
                    else if (Vector2.Distance(NPC.Center, lastKnownTargetPosition) >= 24 * 16)
                    {
                        NPC.velocity += Vector2.Normalize(lastKnownTargetPosition - NPC.Center) * speed;
                    }

                    if (attackTimer == 80 || attackTimer == 90 || attackTimer == 100)
                    {
                        Vector2 num712 = (Vector2.Normalize(Main.player[NPC.target].Center - NPC.Center) * 5f).RotatedByRandom(0.1f);

                        int num228 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, num712, ModContent.ProjectileType<blastermissile>(), 40, 0f, Main.myPlayer, ai0: NPC.target);
                        NPC.netUpdate = true;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Content/NPCs/Monsters/TheVault/blasterglow").Value, position, rect, glowColor * 255, NPC.rotation, Vector2.Zero, 1f, NPC.direction == 1 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0f);
        }
