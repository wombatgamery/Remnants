...
namespace Remnants.Content.World
{
    public class RemWorld : ModSystem
    {
        ...
        {
            #region decoration
            InsertPass(tasks, new Grass("Grass and Moss", 1), FindIndex(tasks, "Moss"), true);
    ...
    public class MiscTools : ModSystem
    {
        ...
        public static void Terraform(Vector2 position, float size, bool[] tilesToDestroy = null, float scaleX = 2, float scaleY = 1, bool killWall = false)
        {
            ...
            if (tilesToDestroy == null)
            {
                ...
                tilesToDestroy[ModContent.TileType<Sulfurstone>()] = true;
                tilesToDestroy[ModContent.TileType<SpiderEggs>()] = true;
                tilesToDestroy[ModContent.TileType<InsectRemains>()] = true;
                ...
                tilesToDestroy[ModContent.TileType<SulfurstoneDiamond>()] = true;
            ...
            for (int y = (int)(position.Y - size * 2 * scaleY); y <= position.Y + size * 2 * scaleY; y++)
            {
                for (int x = (int)(position.X - size * 2 * scaleX); x <= position.X + size * 2 * scaleX; x++)
                {
                    ...
                    {
                        if (noise.GetNoise(x / scaleX, y / scaleY) / 0.8f <= threshold)
                        {
                            if (tilesToDestroy[tile.TileType])
                            {
    ...
    public class StructureTools
    {
        ...
        public static void GenericLoot(int chestIndex, List<(int type, int stack)> itemsToAdd, int grade = 0, int[] uniquePotions = null, bool haveRestorationPotions = false)
        {
            ...
            {
                if (th.TryFind("SteelArrow", out ModItem arrow))
                {
                    steelArrow = arrow.Type;
                }
                if (th.TryFind("SpringWater", out ModItem heal))
                {
                    springWater = heal.Type;
                }
            }
            ...
            if (grade > 0)
            {
                ...
                if (grade < 3 && Main.rand.NextBool(odds) && !magicalLab)
                {
                    ...
                    else if (biomes.FindBiome(x, y) == BiomeID.AquaticDepths)
                    {
                        itemsToAdd.Add((piece == 3 ? ItemID.GoldBroadsword : piece == 2 ? ItemID.GoldGreaves : piece == 1 ? ItemID.GoldChainmail : ItemID.GoldHelmet, 1));
                    }
                ...
                if (Main.rand.NextBool(2))
                {
                    ...
                    else itemsToAdd.Add((y < Main.worldSurface ? ItemID.Grenade : biomes.FindBiome(x, y) == BiomeID.Desert ? ItemID.ScarabBomb : x < 350 || x > Main.maxTilesX - 350 ? ItemID.BombFish : ItemID.Bomb, Main.rand.Next(5, 10)));
                }
                ...
                if (grade < 3)
                {
                    ...
                    if (Main.rand.NextBool(2))
                    {
                        ...
                        {
                            itemsToAdd.Add((steelArrow, Main.rand.Next(25, 50)));
                        }
                        else if (grade <= 1)
                        {
            ...
            while (count > 0)
            {
                ...
                if (!MiscTools.Tile(x, y).HasTile && MiscTools.Tile(x, y).LiquidAmount == 0 && !MiscTools.Solid(x, y - 1) && !MiscTools.Solid(x, y - 2) && MiscTools.Solid(x, y + 1) && !MiscTools.Tile(x, y + 1).HasActuator && TileID.Sets.TouchDamageImmediate[MiscTools.Tile(x, y + 1).TileType] == 0 && MiscTools.Tile(x, y + 1).TileType != TileID.TrapdoorClosed)
                {
                    if (!temple)
                    {
                        ...
                        if (length > 12 && length <= 64 && MiscTools.Tile(trapX, trapY).TileType != TileID.Traps && WorldGen.SolidTile(trapX, trapY + 1) && MiscTools.Tile(trapX, trapY).TileType != TileID.ClosedDoor && MiscTools.Tile(trapX, trapY).TileType != ModContent.TileType<LockedIronDoor>() && TileID.Sets.TouchDamageImmediate[MiscTools.Tile(trapX, trapY).TileType] == 0 && (MiscTools.Tile(trapX, trapY - 1).TileType != TileID.Traps && MiscTools.Tile(trapX, trapY + 1).TileType != TileID.Traps || WorldGen.genRand.NextBool(5)))
                        {
    ...
    public class Safeguard : GenPass
    {
        ...
        public void VerifyCompatibility()
        {
            ...
            {
                issuesFound = true;
                message = message + "\n. " + Language.GetTextValue("Mods.Remnants.Safeguard.LunarVeil");
            }
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            for (int x = 20; x < Main.maxTilesX - 20; x++)
            {
                ...
                for (int y = 20; y < Main.maxTilesY - 20; y++)
                {
                    ...
                    if (tile.HasTile)
                    {
                        ...
                        else
                        {
                            if (tile.TileType == TileID.Stone)
                            {
                                ...
                                {
                                    tile.TileType = (ushort)ModContent.TileType<Sulfurstone>();
                                }
                            }
                            ...
                            else if (biomes.FindBiome(x, y) == BiomeID.Corruption || biomes.FindBiome(x, y) == BiomeID.Crimson)
                            {
                                ...
                                {
                                    if (biomes.FindBiome(x, y) == BiomeID.Crimson)
                                    {
                                        if (tile.TileFrameX < 18 * 3)
                                        {
                                ...
                                else if (TileID.Sets.CountsAsGemTree[tile.TileType])
                                {
                                    WorldGen.KillTile(x, y);
                                    tile.HasTile = false;
                                }
                    ...
                    if (biomes.FindBiome(x, y, false) == BiomeID.Underworld || biomes.FindBiome(x, y, false) == BiomeID.Aether || biomes.FindBiome(x, y) == BiomeID.SulfuricVents || biomes.FindBiome(x, y) == BiomeID.ThermalCaves)
                    {
                    ...
                    else if (y > Terrain.Minimum)
                    {
                        if (biomes.FindBiome(x, y) == BiomeID.Glowshroom || biomes.FindBiome(x, y) == BiomeID.Desert || biomes.FindBiome(x, y) == BiomeID.Savanna || biomes.FindBiome(x, y) == BiomeID.SunkenSea || biomes.FindBiome(x, y) == BiomeID.Marble || biomes.FindBiome(x, y) == BiomeID.Granite || biomes.FindBiome(x, y) == BiomeID.Corruption || biomes.FindBiome(x, y) == BiomeID.Crimson || biomes.FindBiome(x, y) == BiomeID.Beach && y < Main.worldSurface + 50)
                        {
                            tile.LiquidType = 0;
