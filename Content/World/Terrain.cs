...
using Remnants.Content.Tiles.Ocean;
...
namespace Remnants.Content.World
{
    ...
    public class Terrain : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            savannaSpires.SetFractalType(FastNoiseLite.FractalType.None);
            //savannaSpires.SetFractalOctaves(2);
            //savannaSpires.SetCellularJitter(0.75f);
            ...
            FastNoiseLite hellSteps = new FastNoiseLite(WorldGen.genRand.Next(int.MinValue, int.MaxValue));
            hellSteps.SetNoiseType(FastNoiseLite.NoiseType.Cellular);
            hellSteps.SetFrequency(0.025f);
            hellSteps.SetFractalType(FastNoiseLite.FractalType.None);
            hellSteps.SetCellularReturnType(FastNoiseLite.CellularReturnType.CellValue);
            hellSteps.SetCellularJitter(0.75f);

            FastNoiseLite hellChasms = new FastNoiseLite(WorldGen.genRand.Next(int.MinValue, int.MaxValue));
            hellChasms.SetNoiseType(FastNoiseLite.NoiseType.Cellular);
            hellChasms.SetFrequency(0.05f);
            hellChasms.SetFractalType(FastNoiseLite.FractalType.FBm);
            hellChasms.SetCellularReturnType(FastNoiseLite.CellularReturnType.CellValue);
            hellChasms.SetCellularJitter(0.75f);

            FastNoiseLite hellSpikes1 = new FastNoiseLite(WorldGen.genRand.Next(int.MinValue, int.MaxValue));
            hellSpikes1.SetNoiseType(FastNoiseLite.NoiseType.OpenSimplex2S);
            hellSpikes1.SetFrequency(0.025f);
            hellSpikes1.SetFractalType(FastNoiseLite.FractalType.FBm);

            FastNoiseLite hellSpikes2 = new FastNoiseLite(WorldGen.genRand.Next(int.MinValue, int.MaxValue));
            hellSpikes2.SetNoiseType(FastNoiseLite.NoiseType.OpenSimplex2S);
            hellSpikes2.SetFrequency(0.075f);
            hellSpikes2.SetFractalType(FastNoiseLite.FractalType.FBm);
            hellSpikes2.SetFractalGain(0.75f);
            ...
            for (int x = 40; x < Main.maxTilesX - 40; x++)
            {
                ...
                for (int y = 40; y < Main.maxTilesY; y++)
                {
                    Tile tile = Main.tile[x, y];

                    #region terrain
                    float i = x + roughness.GetNoise(x, y + 999) * 10;
                    float j = y + roughness.GetNoise(x + 999, y) * 10 * (y < Main.worldSurface ? Math.Clamp(MathHelper.Distance(x, Desert.OasisX) / 20 - 1, 0, 1) : 1);

                    if (biomes.FindBiome(x, y) == BiomeID.Underworld)
                    {
                    ...
                    if (y < Main.worldSurface)
                    {
                        ...
                        if (biomes.FindBiome(i, j, false) != BiomeID.Beach && biomes.FindBiome(i, j, false) != BiomeID.OceanCave)
                        {
                            ...
                            if (Jungle.Center >= biomes.Width / 2)
                            {
                                mult *= MathHelper.Clamp(MathHelper.Distance(x, MathHelper.Clamp(x, 0, oceanSpan)) / (150 * scaleX), 0, 1);
                            }
                            else mult *= MathHelper.Clamp(MathHelper.Distance(x, MathHelper.Clamp(x, Main.maxTilesX - oceanSpan, Main.maxTilesX)) / (150 * scaleX), 0, 1);
                        ...
                        {
                            _terrain += forestHills.GetNoise(i, 0) / 5 * (1 - beachFactor) * MathHelper.Clamp(forestDistance / (Main.maxTilesX / 4200f), 0, 1) * MathHelper.Clamp(MathHelper.Distance(i / biomes.CellSize, biomes.Width / 2) / (Main.maxTilesX / 2100f), 0, 1);
                        }
                        ...
                        {
                            float _savannaSpires = (-savannaSpires.GetNoise(i, 0) - 0.75f) / 3 * MathHelper.Clamp(forestDistance / (Main.maxTilesX / 4200f), 0, 1);
                        ...
                        {
                            int spacing = 24;// biomes.FindBiome(x, y, false) == BiomeID.Savanna ? 24 : 12;
                            for (int k = 0; k < (biomes.FindBiome(x, y, false) == BiomeID.Savanna ? 2 : 1); k++)
                            {
                                j = j * (1 - Math.Clamp(forestDistance, 0, 1)) + (float)MiscTools.LessSmoothStep((float)Math.Floor((float)j / spacing) * spacing, (float)Math.Ceiling((float)j / spacing) * spacing, (j % spacing) / spacing) * Math.Clamp(forestDistance, 0, 1);
                            }
                        }
                    ...
                    {
                        int max = Main.maxTilesY - 75;
                        int min = Main.maxTilesY - 125;
                        int lavaLevel = Main.maxTilesY - 95;

                        bool ceiling = j < Main.maxTilesY - 150;
                        if (ceiling)
                        {
                            max = Main.maxTilesY - 165;
                            min = Main.maxTilesY - 195;
                        }
                        ...
                        _hellSpikePlacement += 0.5f;
                        float spikeThreshold = 0.4f;

                        if (!ceiling)
                        {
                            _terrain += hellSteps.GetNoise(i, 0) / (ceiling ? 8 : 2);
                        }
                        ...
                        if (!ceiling)
                        {
                            ...
                            if (_terrain > 0.5f)
                            {
                                if (hellChasms.GetNoise(i, 0) > 0)
                                {
                                    _terrain = 99;
                                }
                                else if (_terrain < 0.75f)
                                {
                                    _terrain = 0.5f;
                                }
                            }
                        ...
                        if (ceiling && !forest)
                        {
                            tile.TileType = (ushort)ModContent.TileType<Tuff>();
                        }

                        if (!forest || ceiling)
                        {
                            if (_hellSpikePlacement > spikeThreshold)
                            {
                                if (_hellSpikePlacement > spikeThreshold + 0.1f && tile.TileType == TileID.Ash && (!forest || ceiling))
                                {
                                    tile.TileType = (ushort)ModContent.TileType<Tuff>();
                                }

                                float _hellSpikes = hellSpikes2.GetNoise(x, 0);
                                if (_terrain < 0.5f && _hellSpikes * _hellSpikePlacement > 0.1f && WorldGen.genRand.NextBool(10) && !ceiling)
                                {
                                    tile.LiquidAmount = 255;
                                }

                                _terrain += _hellSpikes * (_hellSpikePlacement - spikeThreshold) / (1 - spikeThreshold) * 1.5f;
                            }
                        }

                        float threshold = _terrain * (max - min) + min;

                        if (j >= threshold && !ceiling || ceiling && j <= threshold)
                        {
                            tile.HasTile = true;
                        }
                        else
                        ...
                        if (_hellSpikePlacement > spikeThreshold)
                        {
                            _terrain += hellSpikes2.GetNoise(x + 999, 0) * (_hellSpikePlacement - spikeThreshold) / (1 - spikeThreshold) * 6 * (ceiling ? -1 : 1);

                            threshold = _terrain * (max - min) + min;

                            if (j >= threshold && !ceiling || ceiling && j <= threshold)
                            {
                                tile.WallType = corruptionRoots.GetNoise(x / 2f, y / 2f) > 0.8f ? WallID.LavaUnsafe3 : (ushort)ModContent.WallType<TuffWall>();
                            }
                        }
                        ...
                        if (forest && ceiling)
                        {
                            if (_hellSpikePlacement < spikeThreshold + 0.1f)
                            {
            ...
            {
                int x = count == 0 ? (int)((Desert.Center + 0.5f) * biomes.CellSize) + WorldGen.genRand.Next(-50, 51) : WorldGen.genRand.NextBool(2) ? WorldGen.genRand.Next(400, Main.maxTilesX / 2 - 75) : WorldGen.genRand.Next(Main.maxTilesX / 2 + 75, Main.maxTilesX - 400);
                int y = Minimum;
            ...
            for (int k = 0; k < 5; k++)
            {
                ...
                for (int x = 40; x < Main.maxTilesX - 40; x++) 
                {
                    for (int y = (int)(Main.worldSurface * 0.35f); y < Main.maxTilesY - 40; y = (y > Main.worldSurface && y < Main.maxTilesY - 200 ? Main.maxTilesY - 200 : y + 1))
                    {
                        Tile tile = Main.tile[x, y];
                ...
                for (int x = 40; x < Main.maxTilesX - 40; x++)
                {
                    for (int y = (int)(Main.worldSurface * 0.35f); y < Main.maxTilesY - 40; y = (y > Main.worldSurface && y < Main.maxTilesY - 200 ? Main.maxTilesY - 200 : y + 1))
                    {
                        if (k < 1 || biomes.FindBiome(x, y) != BiomeID.Underworld)
                        {
                            Tile tile = Main.tile[x, y];
            ...
            for (int x = 40; x < Main.maxTilesX - 40; x++)
            {
                for (int y = 40; y < Main.maxTilesY - 250; y++)
                {
                    Tile tile = Main.tile[x, y];

                    if (y > Main.worldSurface && y < Main.maxTilesY - 250 || tile.WallType == WallID.DirtUnsafe || MiscTools.SurroundingTilesActive(x, y) || (tile.WallType == 0 && MiscTools.Tile(x, y - 1).WallType != 0) && biomes.FindBiome(x, y) != BiomeID.Crimson)
                    {
                        int layer = biomes.GetLayer(x, y);
                        ...
                        else if (tile.TileType == TileID.Stone || biomes.FindBiome(x, y, false) == BiomeID.Beach && !MiscTools.SurroundingTilesActive(x, y))
                        {
            ...
            for (int k = 0; k < 4; k++)
            {
                ...
                for (int x = 40; x <= Main.maxTilesX - 40; x++)
                {
                    for (int y = 40; y < Main.maxTilesY - 40; y = (y > Main.maxTilesY - 250 && y < Main.maxTilesY - 200 ? Main.maxTilesY - 200 : y + 1))
                    {
                        if (!MiscTools.SurroundingTilesActive(x, y) && MiscTools.Tile(x, y).WallType != WallID.CrimsonUnsafe2 && MiscTools.Tile(x, y).WallType != WallID.CrimsonUnsafe4)
                        {
                            int adjacentWalls = 0;
                            ...
                            if ((MiscTools.Tile(x - 1, y).WallType != 0 && MiscTools.Tile(x - 1, y).WallType != ModContent.WallType<Walls.dev.nothing>() || MiscTools.Tile(x + 1, y).WallType != 0 && MiscTools.Tile(x + 1, y).WallType != ModContent.WallType<Walls.dev.nothing>()))
                            {
                                if (biomes.FindBiome(x, y) != BiomeID.Underworld || k >= 2)
                                {
                                    if (k >= 1)
                                    {
                                        if (adjacentWalls < 4 - k)
                                        {
            ...
            count = 0; // LAVAFALL
            while (count < Main.maxTilesX / 105)
            {
                int x = WorldGen.genRand.Next(40, Main.maxTilesX - 40);
                int y = WorldGen.genRand.Next(Main.maxTilesY - 190, Main.maxTilesY - 170);

                if (MiscTools.SolidInArea(x - 1, y, x + 1, y + 1))
                {
                    if (!MiscTools.Tile(x - 2, y).HasTile && MiscTools.Tile(x - 2, y).LiquidAmount == 0)
                    {
                        MiscTools.Tile(x - 1, y).IsHalfBlock = true;
    ...
    public class Caves : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            for (int y = 40; y < Main.maxTilesY - 250; y++)
            {
                progress.Set((y - ((int)Main.worldSurface - 40)) / (float)(Main.maxTilesY - 250 - ((int)Main.worldSurface - 40)));
                ...
                for (int x = 60; x < Main.maxTilesX - 60; x++)
                {
                    ...
                    if (tile.HasTile && biomes.FindBiome(x, y) != BiomeID.SunkenSea)
                    {
                        ...
                        if (y > Main.worldSurface * 0.5f)
                        {
                            if (biomes.FindBiome(x, y) == BiomeID.Underworld)
                            {
                                if (y >= Main.maxTilesY - 120)
                                {
                                    rarity = 10;
                                    OreVein(x, y, WorldGen.genRand.Next(16, 24), rarity, TileID.Hellstone, new List<int> { ModContent.TileType<Tuff>() }, 5, 0.55f, 6, 3);
                                }
                            }
    ...
    public class Grass : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            for (int y = 40; y < Terrain.Maximum + 5; y++)
            {
                for (int x = 40; x < Main.maxTilesX - 40; x++)
                {
    ...
    public class Piles : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            if (!calamityEnabled)
            {
                ...
                while (objects < 1)
                {
                    ...
                    {
                        int x = GenVars.dungeonSide == 1 ? WorldGen.genRand.Next(Main.maxTilesX - 220, Main.maxTilesX - 120) : WorldGen.genRand.Next(120, 220);
                        int y = (int)(Main.worldSurface * 0.5f);
                        while ((!MiscTools.HasTile(x, y + 1, TileID.Sand) || !MiscTools.HasTile(x + 1, y + 1, TileID.Sand)) && MiscTools.Tile(x, y + 1).LiquidAmount == 0 && y < Main.worldSurface)
                        {
                            y++;
                        }

                        if (MiscTools.HasTile(x, y + 1, TileID.Sand) && MiscTools.HasTile(x + 1, y + 1, TileID.Sand) && MiscTools.SolidInArea(x - 5, (int)Main.worldSurface - 60, x + 6, (int)Main.worldSurface - 60))
                        {
                            WorldGen.PlaceObject(x, y, (ushort)ModContent.TileType<Moai>(), direction: GenVars.dungeonSide * -1);

                            if (MiscTools.HasTile(x, y, ModContent.TileType<Moai>()))
                            {
                                objects++;
                                attempts = 0;
                            }
            ...
            for (int y = 40; y < Main.maxTilesY - 40; y++)
            {
                ...
                for (int x = 40; x < Main.maxTilesX - 40; x++)
                {
                    ...
                    if (!tile.HasTile)
                    {
                        ...
                        {
                            if (tile.TileType == TileID.Sandstone && WorldGen.genRand.NextBool(20))
                            {
                    ...
                    else if (tile.TileType == ModContent.TileType<Tuff>() && tile.Slope == SlopeType.Solid && !tile.IsHalfBlock)
                    {
                        if (!Main.tile[x, y + 1].HasTile)
                        {
                            if ( WorldGen.genRand.NextBool(3))
                            {
                                WorldGen.PlaceObject(x, y + 1, ModContent.TileType<TuffStalac>());
                                if (MiscTools.HasTile(x, y + 1, ModContent.TileType<TuffStalac>()))
                                {
                                    int style = WorldGen.genRand.Next(3);
                                    Main.tile[x, y + 1].TileFrameX = (short)(style * 16);
                                    Main.tile[x, y + 2].TileFrameX = (short)(style * 16);
                                    Main.tile[x, y + 1].TileFrameY = 0;
                                    Main.tile[x, y + 2].TileFrameY = 16;
                                }
                            }
                            else if (WorldGen.genRand.NextBool(2))
                            {
                                WorldGen.PlaceObject(x, y + 1, ModContent.TileType<TuffStalacSmall>());
                                if (MiscTools.HasTile(x, y + 1, ModContent.TileType<TuffStalacSmall>()))
                                {
                                    int style = WorldGen.genRand.Next(3);
                                    Main.tile[x, y + 1].TileFrameX = (short)(style * 16);
                                    Main.tile[x, y + 1].TileFrameY = 0;
                        ...
                        if (!Main.tile[x, y - 1].HasTile)
                        {
                            if (WorldGen.genRand.NextBool(3))
                            {
                                ...
                                if (MiscTools.HasTile(x, y - 1, ModContent.TileType<TuffStalac>()))
                                {
                                    ...
                                    Main.tile[x, y - 1].TileFrameX = (short)(style * 16);
                                    Main.tile[x, y - 2].TileFrameX = (short)(style * 16);
                                    Main.tile[x, y - 1].TileFrameY = 16 * 3;
                                    Main.tile[x, y - 2].TileFrameY = 16 * 2;
                                }
                            }
                            else if (WorldGen.genRand.NextBool(2))
                            {
        ...
        private void LargePile(int x, int y)
        {
            if (!MiscTools.Tile(x, y).HasTile && !MiscTools.Tile(x - 1, y).HasTile && !MiscTools.Tile(x + 1, y).HasTile)
            {
                ...
                else if (tile.TileType == TileID.Ash || tile.TileType == TileID.AshGrass)
                {
                    type = TileID.LargePiles2;

                    style = Main.rand.Next(6, 9);
                }
                else if (Main.wallDungeon[tile.WallType] || tile.TileType == TileID.BoneBlock)
                {
