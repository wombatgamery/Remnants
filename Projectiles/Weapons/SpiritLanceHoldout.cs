...
namespace Remnants.Projectiles.Weapons
{
	public class SpiritLanceHoldout : ModProjectile
	{
        ...
        public override bool PreAI()
		{
			...
			if (!Main.dedServ)
			{
				Dust dust = Dust.NewDustDirect(Projectile.Center - Vector2.One * 2.5f + Projectile.velocity * 16 + Main.rand.NextVector2Circular(16, 16), 5, 5, ModContent.DustType<magicaldust>(), 0, 0);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			if (Main.player[Projectile.owner].statMana < Main.player[Projectile.owner].statManaMax)
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    Dust dust = Dust.NewDustDirect(Projectile.Center - Vector2.One * 2.5f + Projectile.velocity * 16, 5, 5, DustID.Electric, 0, 0);
                    dust.velocity = Main.rand.NextVector2CircularEdge(0.5f, 1).RotatedBy(rotation);
                    dust.noGravity = true;
                }
