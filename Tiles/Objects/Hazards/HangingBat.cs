...
namespace Remnants.Tiles.Objects.Hazards
{
	...
	public class HangingBat : ModTile
	{
        ...
        public override void DrawEffects(int i, int j, SpriteBatch spriteBatch, ref TileDrawInfo drawData)
		{
			...
			{
				int num202 = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 6, 0, 0, 100, default(Color), 2f);
				Main.dust[num202].noGravity = true;
			}
			else if (tile.TileFrameX == 30 * 5 && Main.rand.NextBool(10))
			{
				int num203 = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 67, 0, 0, 90, default(Color), 1.5f);
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			if (tile.TileFrameX == 30 * 4)
			{
				...
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Tiles/Objects/Hazards/HangingBat").Value, new Vector2(i * 16 - (int)Main.screenPosition.X - 6, j * 16 - (int)Main.screenPosition.Y - 2) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY + 32, 28, 30), Color.White, 0f, Vector2.Zero, 1f, effect, 0f);
			}
		}
