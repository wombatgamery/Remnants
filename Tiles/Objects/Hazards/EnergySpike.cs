  ...
  //          {
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			if (tile.TileFrameY == 18)
            {
				...
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Tiles/Objects/Hazards/energyspikevisual1").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y - 16) + zero, new Rectangle(frame * 14, 0, 14, 24), RemTile.GoldenCityLightColour(i, j, red: true), 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Tiles/Objects/Hazards/energyspikevisual2").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y - 16) + zero, new Rectangle(frame * 14, 0, 14, 24), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			}
		}
	}
}