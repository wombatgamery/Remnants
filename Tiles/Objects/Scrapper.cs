...
using Remnants.Items.Placeable;
using Terraria;
...
namespace Remnants.Tiles.Objects
{
  ...
  //      }
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            //Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Tiles/Objects/recyclerglow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 + 2 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, height), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			...
			for (int k = 0; k < 7; k++)
			{
				float x = (float)Terraria.Utils.RandomInt(ref randSeed, -10, 11) * 0.15f;
				float y = (float)Terraria.Utils.RandomInt(ref randSeed, -10, 1) * 0.35f;
				Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Tiles/Objects/recyclerflame").Value, new Vector2((float)(i * 16 - (int)Main.screenPosition.X) + x, (float)(j * 16 - (int)Main.screenPosition.Y + offsetY) + y) + zero, new Rectangle(frameX, frameY, 16, 16), color, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
			}
		}
